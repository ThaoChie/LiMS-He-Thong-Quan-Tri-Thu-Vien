...
| Quản lý Kho sách | CRUD sách (tách riêng từng thao tác), import hàng loạt, quản lý PDF | Core |
| Quản lý Danh mục | CRUD danh mục sách (Category) | Core |
| Tra cứu & Tương tác | Tìm kiếm sách, xem chi tiết, đánh giá sách | User |
| Đề xuất & Phê duyệt | Đề xuất mua sách mới, phê duyệt/từ chối | Operation |
| Lưu thông Sách | Đặt trước, mượn tại quầy, gia hạn, trả, quản lý hàng đợi | Operation |
| Quản lý Phạt | Tạo phiếu phạt, thanh toán phạt, theo dõi nợ | Operation |
| Email Notifications | Thông báo tự động SMTP + Celery Cronjob | Nâng cấp |
| Báo cáo & Thống kê | Dashboard tổng quan, lịch sử mượn/trả cá nhân | Reporting |
...
| Relational DB | MySQL | 8.x | Lưu User, Book, Category, Borrow, Fine, Reservation |
...
| FineReceipt | Phiếu phạt tiền (trễ hạn / hư hỏng / mất sách) |
...
| can\_borrow | Thuộc tính User – bị khóa khi có FineReceipt Unpaid |
| Category | Danh mục phân loại sách (VD: Công nghệ thông tin, Kinh tế, Văn học...) |
| Late Fee | Tiền phạt trả trễ: 5.000 VNĐ × số ngày trễ (BR-23) |
...
|:---|:---|:---|:---|
| Guest (Khách) | Người dùng chưa đăng nhập | / (public) | Xem danh sách sách, đăng nhập |
...
| Thủ thư | Nhân viên thư viện – nghiệp vụ thư viện | /staff/ | CRUD sách, xử lý mượn/trả/phạt, phê duyệt |
...
| UC11 – Xử lý Trả sách / Báo Hỏng / Mất | ✗ | ✗ | ✗ | ✓ | ✗ |
| UC16 – Quản lý Hàng đợi Đặt trước | ✗ | ✗ | ✗ | ✓ | ✗ |
| **Nhóm G: Quản lý Phạt** | | | | | |
| UC17 – Thanh toán Phạt | ✗ | ✓ (xem) | ✓ (xem) | ✓ (xử lý) | ✓ (xem) |
...
Hệ thống LiMS bao gồm 6 quy trình nghiệp vụ chính:
...
4. User đăng xuất (UC19) → Quay về trang chủ
...
7. **Nếu quá hạn:** Tính phạt tự động (BR-23)
8. **Nếu hỏng/mất:** Lập phiếu phạt (BR-18)
...
**Luồng quy trình:**
1. User gửi đề xuất mua sách (UC05)
2. Thủ thư xem danh sách đề xuất → Phê duyệt/Từ chối (UC06)
3. Hệ thống gửi email thông báo kết quả cho User
...
```mermaid
flowchart TD
    A[Reader] ---> B[Gửi đề xuất mua sách]
    B ---> C[Thủ thư xem danh sách chờ]
    C ---> D{Quyết định}
    D --->|Phê duyệt| E[Chuyển trạng thái Approved]
    D --->|Từ chối| F[Chuyển trạng thái Rejected]
    E ---> G[Gửi Email thông báo kết quả]
    F ---> G
```

## 3.7 QT6: Quy trình Xử lý Phạt & Thanh toán

**Mô tả:** Phạt được tạo tự động hoặc bởi Thủ thư, thanh toán tại quầy.

**Luồng quy trình:**
1. Phạt được tạo trong quá trình trả sách (UC11): trễ hạn, hỏng, mất
2. FineReceipt Unpaid → can_borrow bị khóa tự động (BR-19)
3. User xem danh sách phạt của mình (UC17)
4. User đến quầy → Thủ thư xử lý thanh toán phạt (UC17)
5. Sau thanh toán → can_borrow được mở khóa

**UC liên quan:** UC11, UC17

**Biểu đồ Luồng công việc (Workflow Diagram):**
```mermaid
flowchart TD
    A[Thủ thư / Hệ thống] ---> B[Tạo Biên lai phạt]
    B ---> C[Khóa quyền mượn sách tự động]
    C ---> D[Reader xem danh sách phạt chưa đóng]
    D ---> E[Đến quầy thanh toán tiền mặt]
    E ---> F[Thủ thư xác nhận thu tiền]
    F ---> G[Tự động mở khóa quyền mượn sách]
```
...
| **Luồng chính (Main Flow) – Vô hiệu hóa** | 1. Admin bấm "Vô hiệu hóa" trên tài khoản mục tiêu.<br>2. Hệ thống kiểm tra tài khoản không có BorrowRecord chưa hoàn thành (BR-05).<br>3. Hệ thống kiểm tra tài khoản không có FineReceipt Unpaid (BR-05).<br>4. Hệ thống kiểm tra Admin không tự vô hiệu hóa chính mình (BR-27).<br>5. Hiển thị dialog xác nhận: "Bạn có chắc muốn vô hiệu hóa tài khoản [Họ tên]?".<br>6. Admin xác nhận → Hệ thống set is\_active = False.<br>7. Hủy tất cả Session đang active của user đó.<br>8. Hiển thị thông báo "Tài khoản đã bị vô hiệu hóa". |
| **Luồng thay thế (Alt Flow)** | **(A1)** Kích hoạt lại tài khoản: Admin bấm "Kích hoạt" → set is\_active = True → Hiển thị thông báo. |
| **Luồng ngoại lệ (Exception Flow)** | **(E1)** User đang nợ sách (vi phạm BR-05) → Báo lỗi "Không thể vô hiệu hóa: tài khoản đang có sách mượn chưa trả".<br>**(E2)** User đang nợ phạt (vi phạm BR-05) → Báo lỗi "Không thể vô hiệu hóa: tài khoản đang có phiếu phạt chưa thanh toán".<br>**(E3)** Admin tự vô hiệu hóa mình (vi phạm BR-27) → Báo lỗi "Không thể vô hiệu hóa tài khoản của chính bạn". |
| **Hậu điều kiện** | Trạng thái is\_active được thay đổi. Nếu vô hiệu hóa: session bị hủy, user không thể đăng nhập. |
| **Business Rules** | BR-05: Không vô hiệu hóa tài khoản đang có BorrowRecord chưa hoàn thành hoặc FineReceipt Unpaid. <br>BR-27: Admin không được tự vô hiệu hóa chính mình. |
...
| 6 | Giá bìa (VND) | Currency | Có | Số thực > 0; dùng tính phạt mất sách (BR-18) | 150000 |
...
| # | Tên trường | Kiểu | Bắt buộc | Validation Rules | Ví dụ / Ghi chú |
|:---:|:---|:---|:---:|:---|:---|
| 1 | Tên sách | Text | Có | Max 255 chars; hệ thống tự so khớp với kho sách | Clean Code |
| 2 | Tác giả | Text | Có | Max 255 chars | Robert C. Martin |
| 3 | Lý do đề xuất | Textarea | Có | Min 20 chars; Max 1000 chars | Sách rất cần cho môn Kỹ thuật phần mềm... |
...
| **Luồng chính (Main Flow)** | 1. Thủ thư mở danh sách đề xuất đang Pending (có thể lọc theo trạng thái, ngày tạo).<br>2. Thủ thư click xem chi tiết phiếu: Tên sách, Tác giả, Lý do, Người đề xuất, Ngày gửi.<br>3. Thủ thư bấm "Phê duyệt".<br>4. Hệ thống cập nhật PurchaseRequest.status = Approved.<br>5. Hệ thống gửi Email thông báo kết quả đến User (qua Celery).<br>6. Hiển thị thông báo "Đã phê duyệt thành công". |
...
| **Tác nhân** | Guest, User (Sinh viên / Giảng viên), Thủ thư, Admin |
...
| **Tiền điều kiện** | Không yêu cầu đăng nhập (Guest được phép xem – BR-30). Đối với User đã đăng nhập: hiển thị thêm nút Đặt trước/Mượn. |
...
| **Business Rules** | BR-11: Tìm kiếm fulltext không phân biệt hoa/thường, tự trim whitespace.<br> BR-30: Guest được phép xem danh sách sách mà không cần đăng nhập. |
...
| **Tiền điều kiện** | Sách đang có available\_copies = 0. User đã đăng nhập. Tài khoản không có FineReceipt Unpaid (BR-13). |
| **Luồng chính (Main Flow)** | 1. User bấm nút "Đặt trước".<br>2. Hệ thống kiểm tra: User không có BorrowRecord Overdue.<br>3. Hệ thống kiểm tra: User không có FineReceipt Unpaid (BR-13).<br>4. Hệ thống kiểm tra: User chưa đạt giới hạn đặt trước đồng thời (BR-12).<br>5. Hệ thống kiểm tra: User chưa đặt trước cuốn sách này.<br>6. Tạo Reservation: status = Waiting, queue\_position = max + 1.<br>7. Hiển thị vị trí xếp hàng.<br>8. Khi có sách trả về: Gửi Email tự động cho người đầu hàng đợi. |
...
| **Luồng ngoại lệ (Exception Flow)** | **(E1)** User có sách quá hạn → Báo lỗi.<br>**(E2)** User nợ phạt (BR-13) → Báo lỗi.<br>**(E3)** Vượt giới hạn (BR-12) → Báo lỗi.<br>**(E4)** Đã đặt trước cuốn này → Báo lỗi.<br>**(E5)** Sách đang có sẵn (race condition) → "Sách hiện có sẵn, hãy mượn trực tiếp". |
| **Hậu điều kiện** | Reservation được tạo với status = Waiting. User nhận Email khi đến lượt (48h ưu tiên – BR-15). |
| **Business Rules** | BR-12: SV tối đa 2 đặt trước; GV tối đa 3.<br> BR-13: FineReceipt Unpaid → cấm đặt trước. |
...
| **Tiền điều kiện** | Sách có available\_copies > 0 HOẶC User đứng đầu hàng đợi Reservation trong 48h ưu tiên (BR-15). Tài khoản User không có FineReceipt Unpaid. |
| **Luồng chính (Main Flow)** | 1. Thủ thư nhập Mã SV/GV và Mã sách/ISBN (hoặc quét mã vạch).<br>2. Hệ thống tra cứu User và Book, hiển thị thông tin xác nhận: Tên User, Tên sách, Số sách đang mượn, Hạn mức còn lại.<br>3. Hệ thống kiểm tra User không nợ phạt và chưa vượt hạn mức (BR-14).<br>4. Thủ thư bấm "Xác nhận Mượn".<br>5. Hệ thống tạo BorrowRecord: status = borrowed; due\_date = now() + số ngày theo vai trò (BR-14).<br>6. Giảm available\_copies đi<br>1.<br>7. Nếu User đứng đầu hàng đợi Reservation → Cập nhật Reservation.status = Completed.<br>8. Gửi Email xác nhận mượn kèm ngày trả (Celery).<br>9. Hiển thị phiếu mượn (có thể in). |
...
| **Luồng ngoại lệ (Exception Flow)** | **(E1)** User nợ phạt → Từ chối.<br>**(E2)** Vượt hạn mức (BR-14) → Báo lỗi.<br>**(E3)** Sách trong 48h ưu tiên của người khác (BR-15) → Từ chối.<br>**(E4)** Mã SV/GV không tồn tại → Báo lỗi.<br>**(E5)** Mã sách không tồn tại hoặc đã hết → Báo lỗi. |
...
| # | Tên trường | Kiểu | Bắt buộc | Validation Rules | Ví dụ / Ghi chú |
|:---:|:---|:---|:---:|:---|:---|
| 1 | Mã Sinh viên/GV | Text | Có | Tồn tại trong DB; is\_active = True; can\_borrow = True | SV2022001 |
...
| **Tiền điều kiện** | BorrowRecord đang ở trạng thái borrowed. due\_date >= ngày hiện tại. Tài khoản không có FineReceipt Unpaid (BR-19). |
...
| **Luồng thay thế (Alt Flow)** | Không có luồng thay thế. |
| **Luồng ngoại lệ (Exception Flow)** | **(E1)** Còn > 2 ngày đến hạn (BR-24) → Nút disabled; HTTP 400 nếu gọi API.<br>**(E2)** Sách có người đặt trước (BR-17) → Nút disabled + tooltip.<br>**(E3)** Đã gia hạn 1 lần (BR-16) → "Bạn đã sử dụng lượt gia hạn".<br>**(E4)** Sách đã quá hạn → "Sách đã quá hạn, vui lòng ra quầy nộp phạt". |
| **Hậu điều kiện** | BorrowRecord cập nhật: due\_date mới; renewal\_count = 1. |
| **Business Rules** | BR-16: Mỗi phiếu mượn gia hạn 1 lần, thêm 10 ngày. BR-17: Gia hạn bị vô hiệu khi sách có Reservation Waiting.<br> BR-24: Nút gia hạn chỉ Enable khi còn ≤ 2 ngày đến hạn. |
...
| **Tác nhân** | Thủ thư |
| **Mô tả** | Thủ thư xử lý thu hồi sách tại quầy: trả bình thường, lập phiếu phạt nếu hỏng, xử lý báo mất với đền bù 150% giá bìa. Phạt trả trễ tính tự động. |
...
| **Luồng chính (Main Flow)** | 1. Thủ thư nhập Mã sách hoặc quét mã vạch.<br>2. Hệ thống tìm BorrowRecord active.<br>3. Hiển thị thông tin phiếu mượn: Người mượn, Ngày mượn, Ngày hẹn trả, Số ngày trễ (nếu có).<br>4. Thủ thư xác nhận sách nguyên vẹn → Bấm "Xác nhận Trả".<br>5. Hệ thống tính số ngày trễ: FineAmount = số ngày trễ × 5.000 VNĐ (BR-23).<br>6. Nếu có phạt trễ → Tạo FineReceipt (reason = Late Fee).<br>7. Cập nhật BorrowRecord.status = returned; available\_copies += 1.<br>8. Nếu có Reservation Waiting → Gửi Email cho người đầu hàng đợi (Celery).<br>9. Hiển thị "Trả sách thành công". |
| **Luồng thay thế (Alt Flow)** | **(A1)** Sách bị hỏng: Thủ thư chọn "Sách hỏng" → Lập FineReceipt (reason = Damaged, amount do Thủ thư nhập) → BorrowRecord = returned; available\_copies += 1.<br>**(A2)** Sách bị mất: Thủ thư chọn "Sách mất" → BorrowRecord.status = lost; total\_copies -= 1; Tạo FineReceipt = Giá bìa × 150% (BR-18); Khóa can\_borrow (BR-19).<br>**(A3)** Trả sách quá hạn: Tự động tính FineAmount = ngày trễ × 5.000 VNĐ (BR-23). |
| **Luồng ngoại lệ (Exception Flow)** | **(E1)** User không chịu đóng phạt mất sách → FineReceipt = Unpaid → Khóa can\_borrow vĩnh viễn (BR-19).<br>**(E2)** Mã sách không tìm thấy BorrowRecord active → Báo lỗi. |
| **Hậu điều kiện** | BorrowRecord hoàn thành (returned/lost). available\_copies và total\_copies cập nhật. FineReceipt tạo nếu có phạt. |
| **Business Rules** | BR-18: Đền bù mất sách = Giá bìa × 150%.<br> BR-19: FineReceipt Unpaid → khóa can\_borrow.<br> BR-23: Phạt trễ = ngày trễ × 5.000 VNĐ/ngày. |
...
| 3 | Ghi chú phạt | Textarea | Có (khi Hỏng) | Max 500 chars | Bìa rách, thiếu 5 trang cuối |
| 4 | Số tiền phạt hỏng | Currency | Có (khi Hỏng) | > 0; Thủ thư tự đánh giá | 50000 |
...
## NHÓM G: QUẢN LÝ PHẠT & THANH TOÁN

---

### UC17 – Thanh toán Phạt

| Mã Use Case | UC17 |
|:---|:---|
| **Tên Use Case** | Thanh toán Phạt |
| **Tác nhân** | User (xem), Thủ thư (xử lý thanh toán) |
| **Mô tả** | User xem danh sách phiếu phạt của mình. Thủ thư xử lý thanh toán tại quầy, cập nhật trạng thái phiếu phạt và mở khóa quyền mượn sách. |
| **Sự kiện kích hoạt** | (User) Truy cập trang "Phiếu phạt của tôi". (Thủ thư) User đến quầy thanh toán, Thủ thư mở Quản lý Phạt. |
| **Tiền điều kiện** | Tồn tại ít nhất 1 FineReceipt cho User (User view) hoặc trong hệ thống (Thủ thư view). |
| **Luồng chính (Main Flow) – Thủ thư xử lý** | 1. Thủ thư mở Quản lý Phạt → Tìm kiếm theo Mã SV/GV hoặc tên.<br>2. Hiển thị danh sách FineReceipt: Mã phiếu, Người bị phạt, Lý do, Số tiền, Trạng thái, Ngày tạo.<br>3. Thủ thư chọn phiếu phạt Unpaid → Bấm "Xác nhận thanh toán".<br>4. Hệ thống cập nhật FineReceipt.status = Paid, ghi nhận ngày thanh toán và phương thức (BR-32).<br>5. Hệ thống kiểm tra: Nếu User không còn FineReceipt Unpaid nào → Mở khóa can\_borrow = True.<br>6. Hiển thị "Thanh toán thành công. Quyền mượn sách đã được khôi phục." |
| **Luồng thay thế (Alt Flow)** | **(A1)** User xem phạt: User truy cập "Phiếu phạt của tôi" → Hiển thị danh sách FineReceipt kèm trạng thái (Paid/Unpaid) → Hướng dẫn đến quầy thanh toán nếu Unpaid. |
| **Luồng ngoại lệ (Exception Flow)** | **(E1)** Phiếu phạt đã thanh toán (Paid) → Không thể thanh toán lại.<br>**(E2)** Không tìm thấy User → Báo lỗi. |
| **Hậu điều kiện** | FineReceipt.status = Paid. can\_borrow được khôi phục nếu không còn phạt Unpaid. |
| **Business Rules** | BR-19: FineReceipt Unpaid → khóa can\_borrow.<br> BR-32: Mỗi lần thanh toán phải ghi nhận phương thức (tiền mặt/chuyển khoản). |

**Bảng Dữ liệu Đầu vào – UC17**

| # | Tên trường | Kiểu | Bắt buộc | Validation Rules | Ví dụ / Ghi chú |
|:---:|:---|:---|:---:|:---|:---|
| 1 | ID Phiếu phạt | Hidden/System | Có | Tồn tại; status = Unpaid | fine\_id = 15 |
| 2 | Phương thức thanh toán | Dropdown | Có | 'Tiền mặt' hoặc 'Chuyển khoản' (BR-32) | Tiền mặt |
...
| **Luồng chính (Main Flow)** | 1. Hệ thống hiển thị Dashboard với các widget thống kê: **[Widget 1]** Tổng quan nhanh: Tổng sách, Tổng user, Đang mượn, Quá hạn, Đặt trước chờ. **[Widget 2]** Sách đang quá hạn: Danh sách BorrowRecord overdue (người mượn, sách, số ngày trễ). **[Widget 3]** Top 10 sách phổ biến: Sách được mượn nhiều nhất (tháng/quý/năm). **[Widget 4]** Thống kê phạt: Tổng doanh thu phạt, Số phiếu Unpaid, Phân loại (Late/Damaged/Lost). **[Widget 5]** Biểu đồ xu hướng mượn: Số lượt mượn theo ngày/tuần/tháng. **[Widget 6]** Đề xuất chờ duyệt: Số PurchaseRequest Pending.<br>2. Tác nhân có thể lọc theo khoảng thời gian (tuần/tháng/quý/năm).<br>3. Tác nhân click vào widget → Điều hướng đến trang chi tiết tương ứng. |
...
| BR-05 | Không vô hiệu hóa tài khoản đang có BorrowRecord chưa hoàn thành hoặc FineReceipt Unpaid. | UC03d | v1.0 |
...
| BR-13 | User đang có FineReceipt Unpaid bị cấm đặt trước sách mới. | UC08 | v1.0 |
...
| BR-18 | Tiền đền bù mất sách = Giá bìa × 150%. | UC11 | v1.0 |
| BR-19 | Tồn tại FineReceipt Unpaid → Tự động khóa thuộc tính can\_borrow của User. | UC11, UC17 | v1.0 |
| BR-20 | Chỉ được Review sách đã mượn VÀ trả thành công. Mỗi User 1 lần review/cuốn. | UC12 | v1.0 |
| BR-23 ★ | Phạt trả trễ = số ngày trễ × 5.000 VNĐ/ngày. Tính tự động khi Thủ thư xác nhận trả sách. | UC11 | v3.5 |
...
| BR-30 ★★ | Guest (chưa đăng nhập) được phép xem danh sách sách và chi tiết sách. | UC07 | v4.0 |
| BR-31 ★★ | Reservation tự động chuyển Expired sau 48h kể từ khi gửi email thông báo (Notified). | UC16 | v4.0 |
| BR-32 ★★ | Mỗi lần thanh toán phạt phải ghi nhận phương thức thanh toán (tiền mặt/chuyển khoản). | UC17 | v4.0 |
...
## 6.1 Các Entity và thuộc tính chính

| Entity / Bảng | Thuộc tính chính | Mô tả |
|:---|:---|:---|
| CustomUser (accounts\_user) | id, username, email, student\_code, full\_name, phone, avatar, role (SV/GV/ThuThu/Admin), is\_active, can\_borrow, created\_at, updated\_at | Kế thừa AbstractUser Django; thêm role, can\_borrow, phone, avatar |
| Category (catalog\_category) | id, name (unique), description, created\_at, updated\_at | Danh mục phân loại sách |
| Book (catalog\_book) | id, title, author, isbn (unique), total\_copies, available\_copies, price, average\_rating, category\_id (FK), publisher, year\_published, created\_at, updated\_at | Metadata sách vật lý |
| PDFDocument (catalog\_pdfdocument) | id, book\_id (FK), file\_path, embed\_status (Pending/Processing/Done/Failed), created\_at, updated\_at | Metadata file PDF; trạng thái embed ChromaDB |
| BorrowRecord (circulation\_borrowrecord) | id, user\_id (FK), book\_id (FK), status (borrowed/returned/lost/overdue), borrowed\_at, due\_date, returned\_at, renewal\_count, created\_at | Phiếu mượn sách |
| Reservation (circulation\_reservation) | id, user\_id (FK), book\_id (FK), status (Waiting/Notified/Completed/Expired/Cancelled), queue\_position, notified\_at, created\_at | Đặt trước sách |
| FineReceipt (circulation\_finereceipt) | id, borrow\_id (FK), user\_id (FK), reason (Damaged/Lost/Late Fee), amount, status (Paid/Unpaid), payment\_method (Cash/Transfer), paid\_at, created\_at | Phiếu phạt tiền |
| PurchaseRequest (proposals\_purchaserequest) | id, user\_id (FK), title, author, reason, status (Pending/Approved/Rejected), reject\_reason, reviewed\_by (FK), reviewed\_at, created\_at | Đề xuất mua sách mới |
| Review (reviews\_review) | id, user\_id (FK), book\_id (FK), rating (1–5), comment, created\_at, updated\_at | Đánh giá sách; UNIQUE (user\_id, book\_id) |

## 6.2 Quan hệ giữa các Entity

| Quan hệ | Mô tả |
|:---|:---|
| Category (1) ─── (N) Book | Mỗi danh mục chứa nhiều sách |
| CustomUser (1) ─── (N) BorrowRecord | Mỗi User có thể có nhiều phiếu mượn |
| Book (1) ─── (N) BorrowRecord | Mỗi sách có thể có nhiều phiếu mượn |
| BorrowRecord (1) ─── (0..N) FineReceipt | Mỗi phiếu mượn có thể phát sinh nhiều phiếu phạt (trễ + hỏng) |
| CustomUser (1) ─── (N) FineReceipt | Mỗi User có thể có nhiều phiếu phạt |
| CustomUser (1) ─── (N) Reservation | Mỗi User có thể đặt trước nhiều sách |
| Book (1) ─── (N) Reservation | Mỗi sách có thể có nhiều người đặt trước |
| CustomUser (1) ─── (N) PurchaseRequest | Mỗi User có thể gửi nhiều đề xuất mua sách |
| CustomUser (1) ─── (N) Review | Mỗi User có thể đánh giá nhiều sách |
| Book (1) ─── (N) Review | Mỗi sách có thể có nhiều đánh giá |

*UNIQUE constraint: Review(user\_id, book\_id) → 1 User chỉ review 1 lần/cuốn (BR-20).*
...
│   │   ├── views.py                # book_search, book_detail (public)
│   │   ├── admin_views.py          # book_create, book_list, book_update, book_delete, book_import
│   │   ├── category_views.py       # category_crud
│   │   └── templates/catalog/
│   ├── circulation/                 # UC08, UC09, UC10, UC11, UC16, UC17
│   │   ├── models.py               # BorrowRecord, Reservation, FineReceipt
│   │   ├── views.py                # reserve_book, renew, borrow_history (user)
│   │   ├── staff_views.py          # process_borrow, process_return, manage_reservations, manage_fines
...
| **D** | UC05 | Gửi yêu cầu mua sách mới | User | P1 – Cao |
| **D** | UC06 | Phê duyệt yêu cầu mua sách | Thủ thư | P1 – Cao |
| **E** | UC07 | Tra cứu & Xem chi tiết Sách | Guest, User, Thủ thư, Admin | P0 – Bắt buộc |
...
| **G** | UC17 | Thanh toán Phạt | User (xem), Thủ thư (xử lý) | P0 – Bắt buộc |
| **H** | UC14 | Dashboard Thống kê | Admin, Thủ thư | P1 – Cao |
| **H** | UC15 | Xem Lịch sử Mượn/Trả cá nhân | User | P0 – Bắt buộc |

**Tổng cộng: 26 Use Cases** (10 UC tách từ bản cũ + 7 UC mới + 9 UC giữ nguyên)
...
## 5.1 Tech Stack (Công nghệ nền tảng)
- **Framework Backend:** Django (MVT Pattern)
- **Frontend / Client:** HTMX, Vanilla CSS (Tối ưu hóa UI/UX mượt mà, không load lại trang)
- **Database:** MySQL
- **Background Jobs:** Celery & Redis (Xử lý tác vụ nặng như Gửi Email nhắc hạn, Cập nhật trạng thái sách tự động)

## 5.2 Sơ đồ Kiến trúc Tổng quan (Architecture Diagram)

```mermaid
flowchart TD
    subgraph Client [Client Tier]
        Browser[Trình duyệt Web]
        HTMX[HTMX (AJAX Calls)]
    end

    subgraph Server [Application Tier - Django]
        Views[Django Views]
    ...
    subgraph Workers [Background Tier]
        Celery[Celery Worker]
        EmailServer[SMTP Email Server]
    end

    Browser <-->|HTTP / AJAX| HTMX
    HTMX <-->|Requests / HTML Responses| Views
    Views <--> Forms
    Views <--> Models
    Models <--> MySQL
    
    Views --->|Đẩy Task vào Queue| Redis
    Redis --->|Lấy Task| Celery
    Celery --->|Thực thi| EmailServer
    Celery <-->|Cập nhật trạng thái| MySQL
```

# 6. THIẾT KẾ DỮ LIỆU (DATABASE SCHEMA)

## 6.1 Sơ đồ ERD Cốt lõi

```mermaid
erDiagram
    User ||--o{ BorrowRecord : makes
    User ||--o{ FineReceipt : pays
    User ||--o{ BookRequest : submits
    
    Book ||--o{ BorrowRecord : "1 cuốn sách có thể mượn nhiều lần (lịch sử)"
    Category ||--o{ Book : categorizes
    
    BorrowRecord ||--o| FineReceipt : generates
...
## 6.2 Data Dictionary (Từ điển Dữ liệu)
| Thực thể | Mô tả nghiệp vụ | Khóa chính (PK) | Thuộc tính quan trọng |
|---|---|---|---|
| **User** | Quản lý tài khoản và phân quyền | `id` | `role` (Quyết định quyền thao tác) |
| **Book** | Thông tin sách trong thư viện | `id` | `available_quantity` (Kiểm soát cho mượn) |
| **BorrowRecord** | Giao dịch mượn/trả sách | `id` | `due_date` (Dùng để tính phí phạt) |
| **FineReceipt** | Biên lai phạt (muộn, hỏng sách) | `id` | `amount`, `status` (Đã đóng/Chưa đóng) |

# 7. ĐẶC TẢ API CỐT LÕI (CORE API SPECS)

Mặc dù hệ thống dùng HTMX, một số API Endpoint nội bộ quan trọng vẫn được xây dựng để xử lý logic:

| Endpoint | Method | Chức năng (Nghiệp vụ) | Input Params |
|---|---|---|---|
| `/api/auth/login/` | POST | Xác thực người dùng | `username`, `password` |
| `/books/search/` | GET | Tìm kiếm sách (HTMX Trigger) | `q` (Query string), `category_id` |
| `/circulation/borrow/` | POST | Xử lý cho mượn sách tại quầy | `user_id`, `book_id` |
| `/circulation/renew/<id>/` | POST | Gia hạn mượn sách (Online) | `borrow_record_id` |
| `/circulation/return/<id>/`| POST | Xử lý trả sách và check hỏng/mất | `borrow_record_id`, `condition` |

# 8. YÊU CẦU PHI CHỨC NĂNG (NON-FUNCTIONAL REQUIREMENTS)

## 8.1 Performance (Hiệu năng)
- **Thời gian phản hồi (Response Time):** Các thao tác tra cứu sách cơ bản phải trả về kết quả dưới `< 2s`.
- **Tải trọng (Concurrency):** Hệ thống chịu tải được tối thiểu 500 CCU (Concurrent Users) trong mùa thi cử.

## 8.2 Security (Bảo mật)
- **Chống Brute-force:** Khóa IP tạm thời (Lockout) trong 15 phút nếu nhập sai mật khẩu 5 lần liên tiếp.
- **Session Timeout:** Phiên đăng nhập tự động hủy sau 30 phút không có thao tác (Idle timeout).
- **Phân quyền (RBAC):** Bắt buộc check Role (Decorator) ở mọi API Endpoint và View. Reader không bao giờ gọi được hàm của Librarian.

## 8.3 Reliability (Độ tin cậy)
- Mọi tác vụ gửi Email (Đòi nợ, Thông báo đặt sách) đều phải qua **Celery Queue**. Nếu SMTP Server chết, hệ thống phải lưu vào Redis và tự động Retry tối đa 3 lần.
- Tính nhất quán dữ liệu (ACID): Các thao tác như (1) Trừ số lượng sách và (2) Tạo BorrowRecord phải nằm trong cùng một `Database Transaction`.

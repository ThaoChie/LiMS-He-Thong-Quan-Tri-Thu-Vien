    ...
    if request.method == 'POST':
        ...
        if action in ['approved', 'rejected']:
            ...
            try:
                ...
                subject = f'Kết quả đề xuất sách: {proposal.title}'
                if action == 'approved':
                    msg = f'Xin chào {proposal.user.username},\n\nĐề xuất mua cuốn sách "{proposal.title}" của bạn đã được thư viện phê duyệt và sẽ sớm được bổ sung vào kho sách. Cảm ơn sự đóng góp của bạn!'
                else:
                    msg = f'Xin chào {proposal.user.username},\n\nRất tiếc, đề xuất mua cuốn sách "{proposal.title}" của bạn đã bị từ chối với lý do:\n"{proposal.admin_notes}"\n\nCảm ơn sự đóng góp của bạn!'
                send_mail(subject, msg, getattr(settings, 'DEFAULT_FROM_EMAIL', 'noreply@lims.local'), [proposal.user.email])
            except Exception:
                pass
